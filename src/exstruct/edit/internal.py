...
def _apply_openpyxl_create_chart(
    ...
    if op.chart_type is None or op.data_range is None or op.anchor_cell is None:
        raise ValueError("create_chart requires chart_type, data_range, and anchor_cell.")

    chart_class_map: dict[str, type] = {
        "line": LineChart, "column": BarChart, "bar": BarChart,
        "area": AreaChart, "pie": PieChart, "doughnut": DoughnutChart,
    ...
    if normalized_type is None or normalized_type not in chart_class_map:
        raise ValueError(f"create_chart chart_type must be one of: {SUPPORTED_CHART_TYPES_CSV}.")
    ...
    chart_summary = f"type={op.chart_type};data={data_summary};anchor={op.anchor_cell};name={op.chart_name or 'Chart'}"
...
def _build_shape_xml(spec: _ShapeSpec, shape_id: int, *, xdr_prefix: str = "xdr:") -> str:
    ...
    fill_xml = (
        ...
        if spec.fill_color
        else '<a:solidFill><a:schemeClr val="accent1"/></a:solidFill>'
    )
    text_xml = ""
    if spec.text:
...
def _apply_openpyxl_create_shape(
    ...
    pending_shapes.append(_ShapeSpec(
        sheet=op.sheet, shape_type=op.shape_type,
        anchor_col=col, anchor_row=row,
        width_emu=width_emu, height_emu=height_emu,
        text=op.text, fill_color=fill_color,
    ))

    summary = f"type={op.shape_type};anchor={op.anchor_cell}"
    if op.text:
        summary += f";text={op.text}"
    return PatchDiffItem(
        op_index=index, op=op.op, sheet=op.sheet, cell=op.anchor_cell,
        before=None, after=PatchValue(kind="shape", value=summary),
    ), None
