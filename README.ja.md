...
### upstream（COM なし）からの改善点

オリジナルの upstream ExStruct では、COM が利用できない環境（Linux/macOS）では **図形とチャートは空配列** でした。本フォークの OOXML パーサーにより、ほぼ完全な機能を提供します：

| 機能 | upstream (COM なし) | 本フォーク (OOXML) |
|------|---------------------|-------------------|
| セル | ✓ | ✓ |
| テーブル候補 | ✓ | ✓ |
| 印刷範囲 | ✓ | ✓ |
