...
- [MOD-02] 型が `dev-docs/specs/data-model.md` と完全一致
...
- [MODE-05d] GitHub Actions の Windows smoke job は `windows-2025` + `libreoffice-fresh` で `EXSTRUCT_LIBREOFFICE_PATH` は `soffice.com` 優先（存在しない場合は `soffice.exe` fallback）とし、`pytest.mark.libreoffice` smoke を skip なしで実行する
